...
export default function Notificaciones({ tareas: tareasProp }: { tareas?: any[] }) {
  const [tareas, setTareas] = useState<any[]>(tareasProp || []);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (tareasProp) {
      // Si ya recibimos tareas como prop desde Pendientes
      setLoading(false);
      return;
    }
    ...
    const q = query(collection(db, 'tareas'), where('uid', '==', auth.currentUser.uid)); // 🔹 mismo campo uid
