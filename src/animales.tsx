...
export default function Animales() {
  ...
  const [animalVacunasModal, setAnimalVacunasModal] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const off = onAuthStateChanged(auth, async (user) => {
      ...
      //Con esta madre se filtra por usuario 
      const catsSnap = await getDocs(
        query(collection(db, 'categorias'), where('uid', '==', user.uid))
      );
      ...
      for (const d of catsSnap.docs) {
        ...
        const snaps = await getDocs(
          query(collection(db, 'categorias', cat, 'animales'), where('uid', '==', user.uid))
        );
    ...
    const printContent = `
      <html>
        <head>
          ...
          <style>
            body { font-family: 'Roboto', sans-serif; margin: 40px; color: #333; background: #fff; }
            header { text-align: center; margin-bottom: 40px; }
            header h1 { font-weight: 700; font-size: 28px; color: #2c3e50; margin-bottom: 5px; }
            header p { font-size: 14px; color: #7f8c8d; }
            .vacuna-card { border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); padding: 20px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; }
            .vacuna-card.aplicada { background-color: #dff0d8; border-left: 6px solid #3c763d; }
            .vacuna-card.no-aplicada { background-color: #f2dede; border-left: 6px solid #a94442; }
            .vacuna-info { max-width: 80%; }
            .vacuna-nombre { font-size: 20px; font-weight: 600; margin-bottom: 6px; }
            .vacuna-fecha { font-size: 14px; color: #555; }
            .vacuna-icon { font-size: 30px; margin-left: 15px; }
            footer { position: fixed; bottom: 30px; width: 100%; text-align: center; font-size: 12px; color: #999; }
          </style>
        </head>
        <body>
          <header>
          ...
          ${animalVacunasModal.vacunas.map((v: any) => `
  ...
  const recargarConVacunas = async () => {
    ...
    const refAnimales = query(
      collection(db, 'categorias', categoriaSeleccionada, 'animales'),
      where('uid', '==', user.uid)
    );
  ...
  const guardarAnimal = async () => {
    ...
    await setDoc(refAnimalDoc, { ...datos, sexo: sexoNorm, uid: user.uid });
  ...
  const agregarCategoria = async () => {
    ...
    await setDoc(doc(db, 'categorias', nueva), { uid: user.uid });
                      ...
                      <td className="py-2 px-3 text-left text-sm">
                        <button
                          ...
                          className={`underline font-medium ${
                            anim.vacunas?.length > 0 && anim.vacunas.every((v: any) => v.aplicada)
